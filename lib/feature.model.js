...
const { uniq, sortedUniq } = require('@lykmapipo/common');
const { getString, getStrings } = require('@lykmapipo/env');
const { Schema, Mixed } = require('@lykmapipo/mongoose-common');
...
const { CONTINENT_NAMES, COUNTRY_NAMES } = require('@lykmapipo/common');
const { MAP_FEATURE_GROUPS } = require('@lykmapipo/common');
...
const DEFAULT_LEVEL = getString('DEFAULT_ADMIN_LEVEL', 'other');
const LEVELS = uniq([
  ADMIN_LEVEL_3_NAME, ADMIN_LEVEL_4_NAME, ADMIN_LEVEL_5_NAME,
  ADMIN_LEVEL_6_NAME, ADMIN_LEVEL_7_NAME, ADMIN_LEVEL_8_NAME,
  ADMIN_LEVEL_9_NAME, ADMIN_LEVEL_10_NAME, ADMIN_LEVEL_11_NAME,
...
const TYPES = sortedUniq(getStrings('FEATURE_TYPES', [
  ...
  'Waterways', 'Facilities', 'Administrative', DEFAULT_TYPE
]));
...
const FeatureSchema = new Schema({
  /**
   * @name group
   * @description Human readable group(or class) of a feature.
   *
   * It may have one by one relationship with OSM primary features key.
   *
   * @type {object}
   ...
   *
   * It may have one by one relationship with OSM primary features value.
   *
   * @type {object}
   * @property {object} type - schema(data) type
   * @property {boolean} trim - force trimming
  ...
  /**
   * @name level
   * @description Human readable administrative level of a feature.
   *
   * @type {object}
   * @property {object} type - schema(data) type
   * @property {boolean} trim - force trimming
   ...
   * @instance
   * @example
   * city, state, region, district etc.
   */
  level: {
    type: String,
    trim: true,
...
FeatureSchema.statics.GROUPS = MAP_FEATURE_GROUPS;
...
FeatureSchema.statics.DEFAULT_LEVEL = DEFAULT_LEVEL;
FeatureSchema.statics.LEVELS = LEVELS;
  ...
  async.waterfall([
    ...
    function findExistingFeature(next) {
      ...
      criteria = (
        criteria._id ?
        _.pick(criteria, '_id') :
        _.pick(criteria, 'group', 'type', 'level', 'name')
      );
...
module.exports = model(FEATURE_MODEL_NAME, FeatureSchema);
