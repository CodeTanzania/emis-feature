...
/* admin levels */
/* @todo provide list per each level */
...
/* feature categories */
const DEFAULT_FEATURE_CATEGORY = getString('DEFAULT_FEATURE_CATEGORY', 'Other');
...
/* feature types */
const DEFAULT_FEATURE_TYPE = getString('DEFAULT_FEATURE_TYPE', 'Other');
const FEATURE_TYPES =
  _.orderBy(_.uniq(_.compact(getStrings('FEATURE_TYPES', [
    ...
    'Waterways', 'Facilities', DEFAULT_FEATURE_TYPE
  ]))));


/* schema options*/
const FEATURE_MODEL_NAME = getString('FEATURE_MODEL_NAME', 'Feature');
const FEATURE_COLLECTION_NAME = getString('FEATURE_COLLECTION_NAME',
  'features');
...
const FeatureSchema = new Schema({
   ...
   * @instance
   * @example
   * Stations, Stops, Watercourses etc.
   */
  type: {
    type: String,
    trim: true,
   ...
   *
   * Apply when an alternative name exists, e.g., a street name has different
   * syntax, sometimes even on street signs, although it is not only for
   * street names.
   *
   * @type {object}
   * @property {object} type - schema(data) type
...
FeatureSchema.methods.preValidate = function preValidate(done) {
  ...
  // derive feature tags
  let tags = [this.category, this.type, this.name];
  _.forEach(ADMIN_LEVEL_NAMES, function (adminLevel) {
    tags = [...tags].concat(this[adminLevel]);
  }.bind(this));
