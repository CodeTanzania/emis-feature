 ...
 * @apiSuccess {String} nature=Other Human readable nature of a feature.
 * @apiSuccess {String} family=Other Human readable family of a feature.
 ...
 * a feature.
 * @apiSuccess {Object} [place] A map of key value pairs describing a place
 * where a feature is(or belongs).
 * @apiSuccess {String} [place.continent=Africa] Human readable continent
 * where a feature belongs.
 * @apiSuccess {String} [place.country=Tanzania] Human readable country
 * where a feature belongs.
 ...
 * @apiSuccess {String} data.nature=Other Human readable nature of a feature.
 * @apiSuccess {String} data.family=Other Human readable family of a feature.
 ...
 * name of a feature.
 * @apiSuccess {String} [data.about=undefined] A brief summary about a
 * feature if available i.e additional details that clarify what a feature is.
 * @apiSuccess {Point} [data.centroid=undefined] A geo-point that may be
 * considered as the center of a feature.
 * @apiSuccess {Geometry} data.geometry A geo-geometry representation of
 * a feature.
 ...
 * a feature.
 * @apiSuccess {Object} [data.place] A map of key value pairs describing a place
 * where a feature is(or belongs).
 * @apiSuccess {String} [data.place.continent=Africa] Human readable continent
 * where a feature belongs.
 * @apiSuccess {String} [data.place.country=Tanzania] Human readable country
 * where a feature belongs.
...
/* declarations */
const Feature = include(__dirname, 'feature.model');
...
router.get(PATH_LIST, function getFeatures(request, response, next) {
  ...
  Feature.get(options, function onGetFeatures(error, results) {
    // forward error
    if (error) {
      next(error);
    }
...
router.post(PATH_LIST, function postFeature(request, response, next) {
  ...
  Feature.post(body, function onPostFeature(error, created) {
    // forward error
    if (error) {
      next(error);
    }
...
router.get(PATH_SINGLE, function getFeature(request, response, next) {
  ...
  Feature.getById(options, function onGetFeature(error, found) {
    // forward error
    if (error) {
      next(error);
    }
...
router.patch(PATH_SINGLE, function patchFeature(request, response, next) {
  ...
  Feature.patch(_id, patches, function onPatchFeature(error, patched) {
    // forward error
    if (error) {
      next(error);
    }
...
router.put(PATH_SINGLE, function putFeature(request, response, next) {
  ...
  Feature.put(_id, updates, function onPutFeature(error, updated) {
    // forward error
    if (error) {
      next(error);
    }
...
router.delete(PATH_SINGLE, function deleteFeature(request, response, next) {
  ...
  Feature.patch(_id, patches, function onDeleteFeature(error, deleted) {
    //forward error
    if (error) {
      next(error);
    }
